...
save_par_v1 = "../model_result/pars_v1-2_line.rds"
save_par_v2 = "../model_result/pars_v2_line.rds"
save_par_v3 = "../model_result/pars_v3-1_line.rds"
save_model_v1 = "../model/model_v1-2_line.rds"
save_model_v2 = "../model/model_v2_line.rds"
save_model_v3 = "../model/model_v3-1_line.rds"
...
# Define an linear growth function for Ii
linear_growth <- function(t, I0, r) {
  I0 + r*(t-t0)
}
...
loss <- function(params) {
  ...
  # Calculate I using linear growth function
  years <- t0:b
  I <- sapply(years, linear_growth, I0, r)
...
lower_bounds <- c(1, 0, 0.000001, 0.000001)
upper_bounds <- c(1e6, 1e4, 1, 1)
if(fit_v1){
  init_params <- c(I0 = 1, r = 5, pd = 0.1, pr = 0.2)
...
ggplot(pvo) +
  ...
  ggtitle("Prediction vs. Observation with linear Growth Invasions (I)") +
  xlab("Year") +
  ylab("Number of invasions") +
  theme_bw() +
  guides(color = guide_legend(title = "Series"))
ggsave("../model_graph/pre_invasion_line.png")
...
ggplot(pvo) +
  ...
  ggtitle("Change in Cumulative Recorded Invasions (F(b)) and linear Growth Invasions (I)") +
  xlab("Year 'a'") +
  ylab("Value") +
  theme_bw() +
  guides(color = guide_legend(title = "Series"))
ggsave("../model_graph/cumu_pre_invasion_line.png")