...
save_par_v1 = "../model_result/pars_v1-2_logis_sig.rds"
save_par_v2 = "../model_result/pars_v2_logis_sig.rds"
save_par_v3 = "../model_result/pars_v3-1_logis_sig.rds"
fit_v1 = FALSE
fit_v2 = FALSE
fit_v3 = FALSE
save_model_v1 = "../model/model_v1-2_logis_sig.rds"
save_model_v2 = "../model/model_v2_logis_sig.rds"
save_model_v3 = "../model/model_v3-1_logis_sig.rds"
...
calculate_Fb <- function(a, I, pd_beta, r_pd, pd0, pr, t0, b) {
  Fb <- 0
  for (i in t0:a) {
    # Calculate the product term for (1 - Pd) over the years since the invasion
    pd_product <- prod(1 - sapply(i:(a-1), sigmoid_function, i, pd_beta, r_pd, pd0))
    if (i == a){
      pd_product = 1
    }
    # Calculate Pd for the current year (a-t0+1 since the invasion)
    pd_current <- sigmoid_function(a, i, pd_beta, r_pd, pd0)
    # Calculate the term for the year i and add to Fb
    Fb <- Fb + I[i - t0 + 1] * pd_product * pd_current * (1 - (1 - pr)^(b - a))
  }
  return(Fb)
}
...
loss <- function(params) {
  ...
  Fb_values <- sapply(years, function(a) calculate_Fb(a, I, pd_beta, r_pd, pd0, pr, t0, b))
...
#(K, r, t_m, pd_beta, r_pd, pd_0, pr)
lower_bounds <- c(1, 0.0001, t0, -Inf, 0, 0, 0.000001)
upper_bounds <- c(1e6, 1, 2021, Inf, Inf, Inf, 1)

if(fit_v1){
  init_params <- c(K = 1.904281e+03, r = 2.585832e-02, t_m = 1.872854e+03, pd_beta = 1, pd_rate = 4.343891e-03, pd0 = 0, pr = 1.667608e-01)
