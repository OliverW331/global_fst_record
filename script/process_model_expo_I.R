...
save_par_v1 = "../model_result/pars_v1-2_expo.rds"
save_par_v2 = "../model_result/pars_v2_expo.rds"
save_par_v3 = "../model_result/pars_v3-1_expo.rds"
save_model_v1 = "../model/model_v1-2_expo.rds"
save_model_v2 = "../model/model_v2_expo.rds"
save_model_v3 = "../model/model_v3-1_expo.rds"
...
# Define an exponential growth function for Ii
exponential_growth <- function(t, I0, r) {
  I0 * exp(r * (t - t0))
}

# Formula to calculate Fb
...
loss <- function(params) {
  ...
  # Calculate I using exponential growth function
  years <- t0:b
  I <- sapply(years, exponential_growth, I0, r)
...
predict = function(params, b){
  ...
  # Calculate Fb_values
  Fb_values <- sapply(years, function(a) calculate_Fb(a, I, pd, pr, t0, b))
  data.frame(year = years, pre = Fb_values, invasion = I)
}

# Replace with the actual fitting code for each dataset version
# Example for version 1.2
lower_bounds <- c(1, 0.0001, 0.000001, 0.000001)
upper_bounds <- c(1e6, 1, 1, 1)
if(fit_v1){
  init_params <- c(I0 = 1, r = 0.05, pd = 0.1, pr = 0.2)
...
# Add similar blocks for v2 and v3.1

# Plotting code (example for version 1.2)
...
ggplot(pvo) +
  ...
  ggtitle("Prediction vs. Observation with Exponential Growth Invasions (I)") +
  xlab("Year") +
  ylab("Number of invasions") +
  theme_bw() +
  guides(color = guide_legend(title = "Series"))
ggsave("../model_graph/pre_invasion_expo.png")
...
ggplot(pvo) +
  ...
  ggtitle("Change in Cumulative Recorded Invasions (F(b)) and Exponential Growth Invasions (I)") +
  xlab("Year 'a'") +
  ylab("Value") +
  theme_bw() +
  guides(color = guide_legend(title = "Series"))
ggsave("../model_graph/cumu_pre_invasion_expo.png")