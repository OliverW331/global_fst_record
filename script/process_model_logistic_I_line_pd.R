...
save_par_v1 = "../model_result/pars_v1-2_logis_expo.rds"
save_par_v2 = "../model_result/pars_v2_logis_expo.rds"
save_par_v3 = "../model_result/pars_v3-1_logis_expo.rds"
fit_v1 = FALSE
fit_v2 = FALSE
fit_v3 = FALSE
save_model_v1 = "../model/model_v1-2_logis_expo.rds"
save_model_v2 = "../model/model_v2_logis_expo.rds"
save_model_v3 = "../model/model_v3-1_logis_expo.rds"
...
#Simulate that the probability of detection increases linearly due to abundance
linear_pd <- function(t, pd0, r_pd) {
  pd0 + r_pd * (t - t0)
}

#formula
calculate_Fb <- function(a, I, pd0, r_pd, pr, t0, b) {
  Fb <- 0
  for (i in t0:a) {
    index <- i - t0 + 1
    # Calculate the product term for (1 - Pd) over the years since the invasion
    pd_product <- 1
    for (j in 1:(a - i)) {
      pd_product <- pd_product * (1 - linear_pd(j, pd0, r_pd))
    }
    # Calculate Pd for the current year (a-t0+1 since the invasion)
    pd_current <- exponential_pd(a - i + 1, pd0, r_pd)
    # Add the term to Fb
    Fb <- Fb + I[index] * pd_product * pd_current * (1 - (1 - pr)^(b - a))
  }
  return(Fb)
}
...
loss <- function(params) {
  ...
  Fb_values <- sapply(years, function(a) calculate_Fb(a, I, pd0, r_pd, pr, t0, b))
...
predict = function(params, b){
  ...
  years <- t0:b
  I <- sapply(years, logistic_growth, K, r, t_m)
  pd_values <- sapply(years - t0, linear_pd, pd0, r_pd)
  # Calculate Fb_values
  Fb_values <- sapply(years, function(a) calculate_Fb(a, I, pd0, r_pd, pr, t0, b))
  result = data.frame(year = years, pre = Fb_values, invasion = I, pd = pd_values)
  return(result)
}
### DF1 ###
# Initial parameter values for optimisation
lower_bounds <- c(1, 0.0001, t0, 0.000001, -Inf, 0.000001)
upper_bounds <- c(1e6, 1, 2021, 1, Inf, 1)

if(fit_v1){
  init_params <- c(K = 1.904281e+03, r = 2.585832e-02, t_m = 1.872854e+03, pd0 = 0.1, pd_rate = 0.001, pr = 1.667608e-01)
...
ggplot(pvo) +
  ...
  scale_color_manual(values = c("pre" = "blue", "pre3" = "purple", "pre2" = "green")) +
  ggtitle("Prediction vs. Observation with Logistic Growth Invasions (I) and linear Pd") +
  xlab("Year") +
...
ggplot(pvo) +
  ...
  ggtitle("Change in Cumulative Recorded Invasions (F(b)) and Logistic Growth Invasions (I) and linear Pd") +
  xlab("Year 'a'") +
  ylab("Value") +
  theme_bw() +
  guides(color = guide_legend(title = "Series"))
ggsave("../model_graph/cumu_pre_invasion_logis_expo.png")
...
ggplot(pvo) +
  ...
  # geom_line(aes(x = Year, y = Fb3, color = "F(3)")) +
  # geom_line(aes(x = Year, y = I, color = "I")) +
  geom_line(aes(x = year, y = pd, color = "pd1")) +
  geom_line(aes(x = year, y = pd_v2, color = "pd2")) +
  geom_line(aes(x = year, y = pd_v3, color = "pd3")) +
  scale_color_manual(values = c("pd1" = "blue", "pd2" = "red2", "pd3" = "purple")) +
