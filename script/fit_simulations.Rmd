---
title: "Detection and Reporting Model of Global Speceis First Record Database"
author: "Muchen Wang"
date: "`r Sys.Date()`"
...
## Functions
This section defines key functions used in the simulation model, each tailored to simulate specific aspects of the invasion, detection, and reporting processes.

### Invasion Growth Function
The `invasion_growth` function models the logistic growth of species invasions.

- **Parameters**:
  - `t`: A given year.
  - `K`: Carrying capacity, the maximum population size that the environment can sustain.
  - `r`: Rate of increase, defining how quickly the population approaches its carrying capacity.
  - `t_m`: The year in which the population reaches half of the carrying capacity, marking the inflection point of the logistic curve.
  - `r_d` (currently ignored): Rate of decline post-peak invasion.
  - `t_d` (currently ignored): The year in which the decline begins.

### Sigmoid Function
The `sigmoid_function` simulates the logistic increase in the probability of detection or reporting as the abundance of the species increases.

- **Parameters**:
  - `t`: A given year.
  - `yoi`: Year of the invasion.
  - `beta`: Shapes the logistic curve, affecting how the probability changes over time.
  - `rate`: Rate of increase, determining how quickly the probability approaches 1.
  - `p0`: A starting value, setting the initial probability before any increase.

### Calculate Fb Function
The `calculate_Fb` function estimates the number of species detected in year `a` and reported by year `b`.

- **Parameters**:
  - `t0`: Starting year of the simulation.
  - `a`: Year of detection.
  - `b`: Year of reporting (version of the database).
  - `I`: Array of actual invasions from year `t0` to year `b`.
  - `pd_beta`, `r_pd`, `pd0`: Parameters determining the probability of detection.
  - `pr_beta`, `r_pr`, `pr0`: Parameters determining the probability of reporting.

```{r}
#Assume the growth of invasion is logistic
...
#Apply formula to calculate #sp detected in Td and reported by Tr 
...
#Generate a dataframe that contains the years from t0 to b, predicted #sp and #actual invasions
...
#Plot the fitted and simulated curve from t0 to b for each simulations
...
#find the outliers in the fitted parameters
...
This section generate the simulation based on the parameters randomly generated within a reasonable range.
```{r}
...
```
random_params contains the combinations of parameters randomly generated to simulate the real world.
```{r}
random_params[[1]]
```
results_n contains the n number of simulated results. Each simulations contains years, number of species reported and actual invasions 
```{r}
results_n[[1]]
```

## Fitting
This section examines how well the fitting process of this model is. The default method is used in optim.
...
```
fitted_n includes the information of the fitted parameters.
```{r}
fitted_n[[1]]
```
## plot fitted params and simulated params
```{r}
if (plot_params){
  ...
  long_params_df <- pivot_longer(combined_params_df, 
                                 ...
                                 names_sep = "-")
  ## add these to code if the outliers need to be excluded
  ...
  p1 = ggplot(long_params_df, aes(x = fit, y = sim, color = parameter)) +
...
The results showed that parameters that determine the probability of detection **\( P_d \)** and reporting **\( P_r \)** are fitted better than the ones for "actual invasion" **\( I \)**,  especially `K` and `r`. However, the loss between the fitted and simulated curves is small shown later in the "Prediction" section. Thus, multiple combinations of the parameters can generate curves with little difference
```{r}
  p1
```

## Prediction
Based on the fitted parameters, calculate the estimated number of #sp reported in each year and compare it to the simulated results.
```{r}
if (prediction) {
